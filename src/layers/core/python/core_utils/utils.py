...
from core_utils.regex import numeric

LOG_LEVELS = {"1": "DEBUG", "2": "INFO", "3": "WARNING", "4": "ERROR", "5": "CRITICAL"}


def get_logger(name=None, default_log_level="4"):
    """
    ...
    """
    level_log = LOG_LEVELS.get(os.environ.get("LOG_LEVEL", default_log_level))
    return Logger(
        service=name,
        level=level_log,
...
def get_data_for_quotation_and_reserve(event):
    body_parameters = get_body(event) or {}
    header_parameters = get_header(event) or {}

    authorization = header_parameters.get("Authorization")

    if not authorization:
        raise Exception("Missing header parameters")

    wallet = body_parameters.get("wallet")
    amount = body_parameters.get("amount")
    external_id = body_parameters.get("external_id")

    if not string_is_number(amount):
        raise ValueError("The amount should be a valid numeric value.")

    if not wallet:
        raise Exception("Missing body parameters")

    return authorization, amount, wallet, external_id
...
def string_is_number(value):
    return bool(list(re.finditer(numeric, value, re.MULTILINE)))
