...
def download_file_to_bucket_s3(*, file_name, bucket, object_name=None):
    """
    Upload a file to a bucket.
    ...
    --------
    >>> from core_aws.s3 import upload_file_to_bucket_s3
    >>> download_file_to_bucket_s3(file_name='my-file.txt', bucket='my-bucket')
    ...
    try:
        response = s3.download_file(Filename=file_name, Bucket=bucket, Key=object_name)
    except ClientError as e:
        logging.error(e)
        return False
    logging.debug(str(response))
